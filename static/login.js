...
document.getElementById("loginForm").addEventListener("submit", async function (e) {
    ...
    try {
        ...
        // FIX: Redirect with email as query parameter
        window.location.href = `/myaccount?email=${result.user_data.email}`;

    } catch (error) {
        console.error("Fetch error during login:", error);
        message.style.color = "red";
        message.textContent = "⚠️ Server error! Check FastAPI server.";
    }
});
